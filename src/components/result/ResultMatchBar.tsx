...
export default function ResultMatchBar({ items, title = "학과별 적합도 분포" }: ResultMatchBarProps) {
  return (
    <div className="bg-white rounded-2xl border border-black px-5 py-5 flex flex-col gap-3">
      <h3 className="text-base font-semibold text-black mb-1">{title}</h3>
      {items.map((item, i) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="text-sm text-[#767676] w-4 shrink-0">{i + 1}</span>
          <span className="text-sm font-medium text-black w-24 shrink-0">{item.label}</span>
          <div className="flex-1 h-3 bg-[rgba(142,142,142,0.2)] rounded-full overflow-hidden">
            <div
